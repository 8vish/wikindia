...
angular.service('myAngularApp', function($route, $location, $window) {
    $route.when(
        '/list', { template: '/assets/data/persona/list.html', controller: ListCtrl });
    $route.when(
        '/list/:sort_by', { template: '/assets/data/persona/list.html', controller: ListCtrl });
    $route.when(
        '/persona/:persona_name', { template: '/assets/data/persona/calc.html', controller: CalcCtrl });
...
CalcCtrl.prototype.getRecipes = function(personaName) {
    ...
    var combos = angular.Array.filter(
        arcana3Combos,
        function(x) { return x.result == arcana_; });
...
CalcCtrl.prototype.getArcanaRecipes = function(arcanaName, filterCallback) {
    ...
    var combos = angular.Array.filter(
        arcana2Combos,
        function(x) { return x.result == arcanaName; });
    for (var i = 0, combo = null; combo = combos[i]; i++) {
